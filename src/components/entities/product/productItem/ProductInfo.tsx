...
const ProductInfo = ({ product }: Props) => {
  ...
  return (
    <div className="flex flex-col gap-2 max-w-[800px]">
      ...
      <div className="md:text-base text-sm">{getCutBack(product.description)}</div>
      <div className="flex md:flex-row flex-col md:items-center gap-4">
        <div className="flex gap-2  text-2xl items-center font-medium">
          ...
          {isDiscount && (
            <div className="text-white py-0.5 px-1 rounded text-xl bg-redDark">-{product.discount?.value}%</div>
          )}
        </div>
